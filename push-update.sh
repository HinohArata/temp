...
updates_dir=/data/afterlife_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have afterlife_DEVICE-VERSION-DATE-TYPE-* as format"
...
# Assume afterlife_DEVICE-VERSION-DATE-TYPE-*.zip
...
adb shell "killall id.afterlife.updater 2>/dev/null"
adb shell "sqlite3 /data/data/id.afterlife.updater/databases/updates.db" \
